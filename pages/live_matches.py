...
def get_cached_matches_data(_api):
    """
    Wrapper function to cache the live matches API call.
    The underscore prefix (_api) tells Streamlit not to hash the API object.
    """
    return _api.get_live_matches()

@st.cache_data(ttl=300) # Cache live match list for 5 minutes
def cached_live_matches(_api):
    return _api.get_live_matches()

@st.cache_data(ttl=60) # Cache scorecards for 1 minute
def cached_scorecard(_api, match_id):
